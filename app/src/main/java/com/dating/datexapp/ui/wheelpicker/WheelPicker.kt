package com.dating.datexapp.ui.wheelpicker
...
fun SafeBox(
   ...
   BoxWithConstraints(
      ...
      val result = remember(maxSize, itemSize, unfocusedCount) {
         val totalSize = itemSize.value * (unfocusedCount * 2 + 1)
         if (totalSize <= maxSize.value) {
            unfocusedCount
         } else {
            (((maxSize.value - itemSize.value) / 2f) / itemSize.value).toInt().coerceAtLeast(0)
         }
      }
      content(result)
   }
